//! Familie-J-Zeugenkatalog (Welle W2, Masterplan §2): je Domäne
//! KNOW01–15 Parität 11/11 · Referenz-Cube schließt (voller Pfad +
...
use cce_materialize::family_j_domains::all_profiles;
...
#[test]
fn w2_all_domains_parity_11_of_11() {
...
#[test]
fn w2_reference_cubes_close_all_gates_green() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
...
#[test]
fn w2_negative_cubes_trigger_expected_core_residue() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
...
#[test]
fn w2_domain_kerntest_reanalyze_is_identity() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
        let reference = a.reference_cube();
        let artifact = closed_path(&a, &reference);
