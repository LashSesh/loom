//! Familie-N-Zeugenkatalog (Welle W4, Masterplan §2): je Domäne
//! COM01–12 Parität 11/11 · Referenz-Cube schließt (voller Pfad +
...
use cce_materialize::family_n_domains::all_profiles;
...
#[test]
fn w4_all_domains_parity_11_of_11() {
...
#[test]
fn w4_reference_cubes_close_all_gates_green() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
...
#[test]
fn w4_negative_cubes_trigger_expected_core_residue() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
...
#[test]
fn w4_domain_kerntest_reanalyze_is_identity() {
    for p in all_profiles() {
        let a = DocDomainAdapter::new(p);
        let reference = a.reference_cube();
        let artifact = closed_path(&a, &reference);
